...
UNION
SELECT
    ra.id, u.id, u.name, u.phone,
    'ngo', ra.status, ra.application_payload::text,
    ra.created_at, ra.review_notes, ra.reviewed_at
FROM role_applications ra
INNER JOIN users u ON u.id = ra.user_id
...
WHERE (?::text IS NULL OR application_type = ?)
